...
class AgentLegacy:
    ...
    def __init__(
        ...
        self.tool_search_sementic = ""
        self.last_model_messge = ""
    ...
    def _fetch_tools(self): 
        dynamic_tools = self.tool_manager.retrieve_tools(self.tool_search_sementic, 3)
    ...
    async def stream(self, input_text: str) -> AsyncIterator[StreamChunk]:
        try:
            ...
            self.tool_search_sementic = f"""
            user intentions : {input_text}\n,
            modle last reply: {self.last_model_messge}
            """
            self.last_model_messge = ""
            step_count = 0
            while step_count < self.max_steps:
                ...
                if not tool_calls:
                    self.last_model_messge = full_content
                    return
